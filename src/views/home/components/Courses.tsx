                        ...
                        {courses?.map((course, i) =>
                            ...
                            <SwiperSlide key={i} className="rounded-2xl bg-white">
                                <div className="relative">
                                    ...
                                    {/* COURSE IMAGE */}
                                    <Image src={`/assets/images/${course.image}.jpg`} alt="Course Image" width={420} height={250} className="w-full rounded-t-2xl" />
                                </div>

                                {/* COURSE CONTENT */}
                                <div className="sm:p-7 p-[18px] sm:pb-5 pb-3">
                                    {/* TITLE */}
                                    <h4 className="sm:text-2xl text-sm sm:leading-[36px] leading-[21px] text-[#2C2C2C] font-semibold">{course.title}</h4>

                                    <div className="flex gap-5 justify-between items-center mt-5 mb-10">
                                        {/* AUTHOR DETAILS */}
                                        <div className="flex items-center gap-3">
                                            {/* IMAGE */}
                                            <Image src={`/assets/images/${course.author.img}.png`} alt="User" width={45} height={45} />

                                            {/* NAME AND POSITION */}
                                            <div>
                                                <p className="text-sm font-medium tex-[#2C2C2C] leading-[21px]">{course.author.name}</p>
                                                <p className="text-[10px] leading-[15px] text-gray">{course.author.position}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-[6px]">
                                            ...
                                            <div>
                                                <p className="text-sm font-medium tex-[#2C2C2C] leading-[21px]">{course.participant}</p>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <Button path="#" className="font-semibold text-xs leading-[30px] px-6">Buy Now</Button>
                                        <div className="flex items-end">
                                            <h4 className="text-2xl leading-[36px] font-semibold text-[#2C2C2C]">${course.price}</h4>
                                            <p className="text-xs leading-[30px] text-gray">/{course.videoCounts} Video</p>
                                        </div>
                                    </div>
                                </div>
                            </SwiperSlide>
